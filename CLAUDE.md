...
Yeet scripts can load and run a compiled eBPF object (`.bpf.o`), bind
to its maps, and stream ringbuf events back into JS. Two import forms
are available:

- **`yeet:bpf`** — the virtual module exposing `BpfObject`,
  `BpfControl`, `MapView`, and `BpfSubscription` classes.
- **`./something.bpf.o`** — any file whose path ends with `.bpf.o`
  resolves to a default-exported `BpfObject` already pointing at that
  ELF (no disk read happens at import time; the generated module just
  holds the resolved absolute path).

### Importing an ELF

```js
// Resolves to: export default new BpfObject({ exe: "/abs/.../probe.bpf.o" })
...
const probe = new BpfObject({ exe: "/opt/yeet/probes/probe.bpf.o" });
// Relative `exe` requires `base`:
const probe2 = new BpfObject({ exe: "./probe.bpf.o", base: import.meta.dirname });
```

`exe` must be a string. If `exe` is relative, `base` must be provided
(also a string) — otherwise the constructor throws. Absolute `exe`
paths are normalized (`./` and `..` collapsed).

### Declaring binds and attaches

`BpfObject` is a chainable declarative builder. Nothing is loaded
until `.start()` is awaited.

```js
probe
  .bind("events", { capacity: 4096 })   // bind a map by name + opts
  .attach("handle_exec", {});            // attach a prog by name + opts
```

- `.bind(name, spec)` declares a binding for the named map. Calling
  `.bind()` twice with the same `name` throws.
- `.attach(name, spec)` declares an attachment for the named program.
  Calling `.attach()` twice with the same `name` throws.
- Both methods return `this` for chaining.

The `spec` object is forwarded to the daemon as-is. Keys the daemon
currently honors:

| Map type | Key | Meaning |
|---|---|---|
| `RingBuf` | `capacity` | In-process broadcast buffer size (entries). Falls back to a default if omitted. Bump it for high-volume probes. |

Other map types (e.g. `Array` for `.bss`/`.rodata` data sections)
accept the bind call but currently ignore extra opts. Program
attachment specs are reserved for future use — XDP and TCX progs
attach to every interface today; simple progs use `prog.attach()`
with libbpf defaults.

### Starting and stopping

```js
const ctl = await probe.start();   // returns BpfControl
// ... do work ...
await ctl.stop();                  // tears down the blob
```

`start()` is async and returns a `BpfControl`. `stop()` unloads the
program, unpins maps, and detaches links. The runtime keeps the
underlying blob mounted on the isolate, so forgetting `stop()` will
keep the script alive (no auto-exit).

### Subscribing to ringbuf events

A `BpfControl` exposes a thin per-map view via `.map(name)`. Call
`.subscribe(cb)` on the view to start receiving events:

```js
const sub = await ctl.map("events").subscribe(
  (event) => console.log(event),
  (err)   => console.warn("ringbuf:", err.message),  // optional
);
// ...
await sub.unsubscribe();
```

- `subscribe(cb, onError?)` returns a Promise that resolves to a
  `BpfSubscription` once the daemon-side pump is wired up. It
  rejects if the map name is unknown, the blob is already stopped,
  or registration otherwise fails.
- `cb(event)` receives each decoded event as a plain JS object
  (decoded via the map's BTF).
- `onError(err)` is the runtime-error channel — e.g. a slow consumer
  causing broadcast lag to drop events. Defaults to
  `console.warn(err.message)` if omitted; do **not** rely on the
  default in long-running scripts where dropped events matter.
- `BpfSubscription.unsubscribe()` cancels the stream.

### Lifecycle gotchas

- A failed `start()` rejects with `{ code, message }` — typical
  causes: bad ELF path, verifier rejection, missing CAP_BPF.
- `.bpf.o` imports do not validate the file at import time. Errors
  surface from `start()`.
- The ringbuf subscriber is mounted on the isolate; like any pending
  stream it keeps the isolate alive until `unsubscribe()` (and
  `stop()`) run, so always tear down before letting the top-level
  module finish.
